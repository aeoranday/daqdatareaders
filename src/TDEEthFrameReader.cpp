...
__m256i
TDEEthFrameReader::expand_frame(const __m256i& regi) {
...
void
TDEEthFrameReader::fill_adcs(const fddetdataformats::TDEEthFrame& frame, std::span<uint16_t, s_chunk_size> adc_span) {
  const fddetdataformats::TDEEthFrame::word_t (*words_ptr)[fddetdataformats::TDEEthFrame::s_bits_per_adc] = frame.adc_words;

  uint16_t* adc_ptr = adc_span.data();

  const int register_alignment = fddetdataformats::TDEEthFrame::s_bits_per_adc * 16;

  for (int t = 0; t < s_num_samples_per_frame; t++) {
    const fddetdataformats::TDEEthFrame::word_t *time_sample = *(words_ptr + t);
...
TDEEthFrameReader::get_fragment_crate_slot_stream(const std::string& path) {
  const std::unique_ptr<daqdataformats::Fragment> frag = m_h5_file->get_frag_ptr(path);
  fddetdataformats::TDEEthFrame* frame_ptr = reinterpret_cast<fddetdataformats::TDEEthFrame*>(frag->get_data());
...
std::vector<uint16_t>
TDEEthFrameReader::read_fragment(const std::string& path) {
  std::vector<fddetdataformats::TDEEthFrame> frames = HDF5Reader::read_fragment<fddetdataformats::TDEEthFrame>(path);
...
std::vector<uint16_t>
TDEEthFrameReader::read_all_fragments(const std::vector<std::string>& paths) {
