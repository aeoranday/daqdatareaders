...
class TDEEthFrameReader : public HDF5Reader {
  __m256i expand_frame(const __m256i& regi);


  public:
    static const int s_num_channels_per_frame = fddetdataformats::TDEEthFrame::s_num_channels;
    static const int s_num_samples_per_frame = fddetdataformats::TDEEthFrame::s_time_samples_per_frame;
    static const int s_chunk_size = s_num_channels_per_frame * s_num_samples_per_frame;

    TDEEthFrameReader(const std::string& file_name) : HDF5Reader(file_name) {
    ...
    void fill_adcs(const fddetdataformats::TDEEthFrame& frame, std::span<uint16_t, s_chunk_size> adc_span);
};
